...
// export default function NewAdvancedTable({ data, setSearch, setRowsPerPage, refetch, page, setPage }: { page: number; setPage: React.Dispatch<React.SetStateAction<number>>; refetch: () => void; data: ApiResponse; setSearch: React.Dispatch<React.SetStateAction<string>>; setRowsPerPage: React.Dispatch<React.SetStateAction<number | null>> }) {
...
//   const [rowsPerPageState, setRowsPerPageState] = useState<number>(10);
...
//   const [profile, setProfile] = useState<Users[]>(data.results);
//   const [totalUsers, setTotalUsers] = useState<number>(data.count);
...
//   const headerColumns = visibleColumns === "all" ? columns : columns.filter((column) => Array.from(visibleColumns).includes(column.uid));

//   const filteredItems = profile.filter((user) => {
//     let matchesFilter = true;
//     if (hasSearchFilter) {
//       matchesFilter = user.first_name.toLowerCase().includes(filterValue.toLowerCase());
//     }
//     if (statusFilter !== "all" && Array.from(statusFilter).length !== statusOptions.length) {
//       matchesFilter = matchesFilter && Array.from(statusFilter).includes(user.is_active ? (user.is_blocked ? "active" : "blocked") : "inactive");
//     }
//     return matchesFilter;
//   });

//   const start = (page - 1) * rowsPerPageState;
//   const end = start + rowsPerPageState;

//   const items = filteredItems.slice(start, end);

//   const sortedItems = items.sort((a: Users, b: Users) => {
...
//   const renderCell = (user: Users, columnKey: React.Key) => {
...
//             </DropdownMenu>
//           </Dropdown>
//           <Button className="bg-foreground text-background" endContent={<PlusIcon size={18} />} size="sm">
//             Add New
//           </Button>
...
//           Rows per page:
//           <select className="bg-transparent outline-none text-default-400 text-small cursor-pointer" onChange={onRowsPerPageChange}>
...
//         page={page}
//         total={Math.ceil(totalUsers / rowsPerPageState)}
//         variant="light"
//         onChange={setPage}
//       />
...
//         {selectedKeys === "all"
//           ? "All items selected"
//           : `${selectedKeys.size} of ${profile.length} selected`}
//       </span>
//     </div>
//   );
