...
const DemoSlider = ({ images }:{images:Images[]}) => {
  return (
    <section className="w-full flex items-center justify-center ">
      <span className="relative w-[90vw] flex flex-col h-[500px] m-0 bg-neutral-950 rounded-lg">
        <Swiper
          navigation
    ...
    <>
      {isMobile ? (
        ...
        images.map(({image}:{image:string}, index) => (
          <span
            key={index}
            className="mmd:w-[49%] bg-neutral-950 flex items-center justify-center rounded-xl p-3"
          >
            <Image
