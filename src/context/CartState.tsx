...
export const CartProvider: React.FC<CartProviderProps> = ({ children }) => {
  ...
  const addToCart = async (id: number, event: React.MouseEvent<HTMLButtonElement>, variantId?: number | null) => {
    ...
    const carddataElement = buttonElement.closest('.clone_element') as HTMLElement;
    
    if (!carddataElement) return;

    const mainContent = carddataElement.querySelector('.main-content') as HTMLElement;
    
    const cardClone = carddataElement.cloneNode(true) as HTMLElement;
    const cardRect = carddataElement.getBoundingClientRect();
